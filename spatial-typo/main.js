...
class Genome {
    static createRandom() {
        return {
            ...
            g_fluid: 0, g_mycelium: 0, g_swarm: 0, g_orbit: 0, g_pulse: 0, g_vortex: 0, g_gravity: 0,
            g_speed: 0.1, g_amplitude: 0.2, cohesion: 1.0,
            v_resolution: 0.15, v_roughness: 0, v_fisheye: 0, v_shear: 0, v_complexity: 0.1,
            v_archetype: Math.floor(Math.random() * 4),
            v_neural: Math.random() > 0.5 ? 1 : 0,
            v_membrane: Math.random() > 0.8 ? 1 : 0,
            v_spine: 1,
            v_spores: Math.random() > 0.7 ? 1 : 0,
            v_sharp: Math.random() > 0.9 ? 1 : 0,
            v_liquid: 0, v_glitch: 0,
            v_elementScale: rand(0.5, 1.0), v_elementCount: rand(0.4, 0.8),
            v_strokeW: rand(1.2, 2.5), v_dashA: 0, v_dashB: 0, v_alphaF: 80, v_alphaS: 220,
    ...
    static merge(A, B) {
        const child = {};
        const pPure = Math.random() < 0.6;
        const dom = Math.random() > 0.5 ? A : B;
        const EXCLUSIVE = ['v_archetype', 'v_scaleX', 'v_scaleY', 'v_strokeW', 'blend_additive'];
        const ACCUM = ['g_fluid','g_mycelium','g_swarm','g_orbit','g_pulse','g_gravity','v_roughness','v_glitch'];
        for (const k in A) {
            if (EXCLUSIVE.includes(k)) child[k] = pPure ? dom[k] : (Math.random() > 0.5 ? A[k] : B[k]);
            else if (ACCUM.includes(k)) child[k] = clamp(((A[k]||0)+(B[k]||0))/2 + (Math.random()<0.4?rand(0.05,0.2):0), 0, 5);
            else if (k === 'cohesion') child[k] = clamp(((A[k]||1)+(B[k]||1))/2 * 0.9, 0.1, 0.98);
            else if (k.startsWith('color')) child[k] = clamp(((A[k]||128)+(B[k]||128))/2 + rand(-30,30), 0, 255);
            else if (k.startsWith('v_alpha')) child[k] = ((A[k]||100)+(B[k]||100))/2;
            else child[k] = pPure ? dom[k] : ((A[k]||0)+(B[k]||0))/2;
        }
...
class LivingTypo {
    constructor(p, char, fontData, parentData = null) {
        ...
        if (parentData) {
            ...
            while (this.vertices.length > 550) this.vertices.splice(Math.floor(Math.random()*this.vertices.length), 1);
        } else {
    ...
    draw() {
        ...
        p.push(); p.translate(this.x, this.y); p.scale(d.v_scaleX, d.v_scaleY);
        if (d.blend_additive) p.blendMode(p.ADD);
        p.noFill(); p.stroke(R, G, B, d.v_alphaS); p.strokeWeight(d.v_strokeW);
        p.beginShape(); this.vertices.forEach(v => p.vertex(v.pos.x, v.pos.y)); p.endShape();
        if (d.v_archetype === 0 && d.v_membrane > 0.1) {
            p.noStroke(); p.fill(R, G, B, d.v_alphaF); p.beginShape();
        ...
        if (d.v_archetype === 1 && d.v_sharp > 0.1) {
            ...
            for (let i=0; i<this.vertices.length-2; i+=4) { p.fill(R*0.8, G*0.8, B*0.8, d.v_alphaF*1.5); p.vertex(this.vertices[i].pos.x, this.vertices[i].pos.y); p.vertex(this.vertices[i+1].pos.x, this.vertices[i+1].pos.y); p.vertex(this.vertices[i+2].pos.x, this.vertices[i+2].pos.y); }
            p.endShape();
        }
        if (d.v_archetype === 2 && d.v_spores > 0.1) {
            ...
            for (let i=0; i<this.vertices.length; i+=Math.floor(3/d.v_elementCount)) {
                const v = this.vertices[i]; const sz = (8 + v.seed*30)*d.v_elementScale;
                p.push(); p.translate(v.pos.x, v.pos.y); p.rotate(v.vel.heading()+v.seed*p.TWO_PI);
                if (i%2===0) p.rect(-sz/2, -sz/2, sz, sz); else p.circle(0,0,sz); p.pop();
            }
        }
