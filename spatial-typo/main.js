// Typography Lab - Alchemy Engine v42.0
// DIRECT PORT of v25.1 additive phenotype system
// ONE genome, ONE render pipeline for ALL atoms
// Gen 1 stays letter-shaped via high cohesion + low roughness
// Fusion = average parent genomes → genuine visual filiation
console.log("TypoLab v42.0 — UNIFIED PHENOTYPE | TRUE FILIATION");
...
// ═══════════════════════════════════════════════════════════
// GENOME — ONE genome per atom, same render system for all
// ═══════════════════════════════════════════════════════════
class Genome {
    static createRandom() {
        return {
    ...
    // Biological dominance: strong traits stay strong
    static merge(A, B) {
        const child = {};
        ...
        for (const k in A) {
            ...
            if (PHENOTYPES.includes(k)) {
                // Dominant gene (>0.5) → child inherits strongly
                ...
                // Gentle decay (~12% per generation)
                child[k] = clamp((av + bv) / 2 * 0.88, 0.08, 0.98);
            } else if (k.startsWith('color')) {
                child[k] = clamp((av + bv) / 2 + rand(-25, 25), 0, 255);
...
// ═══════════════════════════════════════════════════════════
// LIVING TYPO — same render pipeline for Gen 1 AND children
// ═══════════════════════════════════════════════════════════
class LivingTypo {
    constructor(p, char, fontData, parentData = null) {
        ...
        if (parentData) {
            ...
            if (fontData && fontData.obj) {
                const b = fontData.obj.textBounds(this.char, 0, 0, 400);
                const pts = fontData.obj.textToPoints(
    ...
    draw() {
        ...
        // PHENOTYPE 2: SPINE (continuous jagged trace)
        if (d.v_spine > 0.01) {
            p.noFill();
        ...
        // PHENOTYPE 3: NEURAL (proximity mesh web)
        if (d.v_neural > 0.01) {
...
class TypoUniverse {
    ...
    checkFusion(moved) {
        const other = APP_STATE.atoms.find(o => o !== moved && Math.hypot(o.x - moved.x, o.y - moved.y) < 180);
        if (!other) return;
        this.history.push([...APP_STATE.atoms]);
    ...
    updateMoleculeList() {
        ...
        ml.innerHTML = APP_STATE.atoms.map(a => {
            ...
            // Show dominant phenotypes
            const traits = [];
            if (d.v_membrane > 0.3) traits.push(`mem:${d.v_membrane.toFixed(1)}`);
            if (d.v_neural   > 0.3) traits.push(`net:${d.v_neural.toFixed(1)}`);
            if (d.v_spine    > 0.3) traits.push(`spi:${d.v_spine.toFixed(1)}`);
            if (d.v_spores   > 0.3) traits.push(`spo:${d.v_spores.toFixed(1)}`);
            if (d.v_sharp    > 0.3) traits.push(`shp:${d.v_sharp.toFixed(1)}`);
            const sub = traits.length ? traits.join(' ') : 'subtle';
